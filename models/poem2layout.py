# models/poem2layout.py (V5.20: BUG FIX - rel_id -> rel)
...
class Poem2LayoutGenerator(nn.Module):
    def __init__(self, bert_path: str, num_classes: int, hidden_size: int = 768, bb_size: int = 64, 
                 ...
                 consistency_loss_weight: float = 1.0, # [INNOVATION] 一致性损失权重
                 latent_dim: int = 32,               
                 **kwargs): 
        ...
        self.consistency_loss_weight = consistency_loss_weight # [New]
        ...
        # 10. [INNOVATION] Dual-Stream Consistency Projection
        # 将 Layout Stream (bb_size) 投影到 Text Semantic Space (hidden_size)
    ...
    # [MODIFIED] Updated signature to accept decoder_output for consistency loss
    def get_loss(self, pred_cls, pred_bbox_ids, pred_boxes, pred_count, layout_seq, layout_mask, num_boxes, target_coords_gt=None, kg_spatial_matrix=None, kg_class_weights=None, kg_class_ids=None, decoder_output=None):
        ...
        # [INNOVATION] Compute Consistency Loss
        ...
        total_loss = self.reg_loss_weight * loss_reg + \
                     ...
                     self.consistency_loss_weight * loss_consistency # Add Consistency Loss
                     
        return total_loss, loss_relation, loss_overlap, \
               loss_reg, loss_iou, loss_size_prior, loss_area, \
               loss_alignment, loss_balance, loss_clustering, loss_consistency # [Ret] Added return item

    # [INNOVATION] New Method: Consistency Loss
    def _compute_consistency_loss(self, decoder_output, mask):
        """
        计算双流一致性损失。
        Decoder输出是 [text_stream, layout_stream] 的拼接。
        我们需要确保这两个流在语义空间上是一致的。
        """
        ...
        # 2. 投影对齐 (Layout -> Text Semantic Space)
        ...
        # Cosine Similarity (target is 1.0)
    ...
    # [FIXED] Updated variable names: rel_id -> rel
